...
public class Tool_TorchLauncher extends ItemTool{

   public Tool_TorchLauncher(ToolMaterial material, String name) {
		...
		{ 		
				...
				if (getMode(stack) == 2 )
				{
					...
					//Skip To Dynamite
							...
							((EntityPlayer) entityLiving).inventory.hasItem(ModRegistry.dynamitebolt))
						{
						entityLiving.worldObj.playSoundAtEntity(entityLiving, "mob.chicken.plop", 1.0F, 3.0F);
						setMode(stack, 6);
						if(getTload(stack)== 2 && !((EntityPlayer) entityLiving).capabilities.isCreativeMode){
							((EntityPlayer) entityLiving).entityDropItem(new ItemStack(Blocks.torch), 0.0F);
							((EntityPlayer)entityLiving).inventory.consumeInventoryItem(ModRegistry.dynamitebolt);
						}
				...
				else if (getMode(stack) == 6)
				{
					...
					if(((EntityPlayer) entityLiving).capabilities.isCreativeMode || 
						...
						setMode(stack, 8);
						if(getTload(stack)== 2 && !((EntityPlayer) entityLiving).capabilities.isCreativeMode){
							((EntityPlayer) entityLiving).entityDropItem(new ItemStack(ModRegistry.dynamitebolt), 0.0F);
							((EntityPlayer)entityLiving).inventory.consumeInventoryItem(Items.arrow);	
						}
						ChatComponentTranslation chatcomponenttranslation = new ChatComponentTranslation(EnumChatFormatting.GRAY + "Arrows loaded", new Object[0]);
						((EntityPlayer) entityLiving).addChatComponentMessage(chatcomponenttranslation);
		   ...
		   switch(getMode(stack))
			{
				case 2:
					...
					par2World.spawnEntityInWorld(new Torch_Projectile(par2World, par3EntityPlayer));
				...
				case 4:
					 //par3EntityPlayer.playSound("mob.chicken.plop", 3F, 3.0F);
					   par2World.spawnEntityInWorld(new RedTorch_Projectile(par2World, par3EntityPlayer));
					   par2World.playSoundAtEntity(par3EntityPlayer, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
					   setTload(stack, 0);
				...
				case 6:
					//par3EntityPlayer.playSound("mob.chicken.plop", 3F, 3.0F);
		       		   par2World.spawnEntityInWorld(new Dynamite_Projectile(par2World, par3EntityPlayer));
		       		   par2World.playSoundAtEntity(par3EntityPlayer, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
		       		   setTload(stack, 0);
	   ...
	   ////////////////////////
	   	if(par3EntityPlayer.isSneaking() && getTload(stack)== 0){
	   		...
	   		//Dynamite
	   				...
	   				par3EntityPlayer.inventory.hasItem(ModRegistry.dynamitebolt) && getMode(stack) == 6){
	   			setTload(stack, 2);
	   			par3EntityPlayer.worldObj.playSoundAtEntity(par3EntityPlayer, "fire.ignite",.4F, itemRand.nextFloat() * 0.4F + 0.8F);
	   			if (!par3EntityPlayer.capabilities.isCreativeMode){                	
					   par3EntityPlayer.inventory.consumeInventoryItem(ModRegistry.dynamitebolt);	
					  //stack.damageItem(3, par3EntityPlayer);
	            	}
	   			stack.damageItem(3, par3EntityPlayer);
	   			return stack;}
