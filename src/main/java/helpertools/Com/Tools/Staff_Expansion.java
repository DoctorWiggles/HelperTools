...
public class Staff_Expansion extends ToolBase_Default
{
    public Staff_Expansion(ToolMaterial material, String unlocalizedName)
    {
    	super (material);
    ...
    {
    	list.add(TextFormatting.WHITE + "Sets blocks in the world");
        list.add(TextFormatting.ITALIC + "While sneaking change mode");
        list.add(TextFormatting.ITALIC + "- Or select block to place");
        list.add(" ");
    ...
    if(whatBlockString(stack) != "null" && whatModeString(stack)!= "null"){
    	list.add(whatBlockString(stack) +returnTMeta(stack) +" "+ whatModeString(stack)+ " mode");
    }}}
	...
	public Set<BlockPos> Pillar_Mode(ItemStack stack, EntityPlayer player, BlockPos pos, EnumFacing theface, boolean simulation){
		...
		{        		
			...
			if(ModUtil.isValid(world, pos2) && getTBlock(stack) != 0){
			...
			{ 
				...
				{ 
					...
					if (world.getBlockState(pos3) == BlockStateHelper.returnState(getTBlock(stack))){
						if(ModUtil.isValid(world, pos4) && getTBlock(stack) != 0){
