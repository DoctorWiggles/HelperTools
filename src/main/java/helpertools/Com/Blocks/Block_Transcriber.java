    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
    {
    	...
    	if (tile != null)
    	{
    		...
    		if(!player.isSneaking()){
    			...
    			switch(side){
    			case DOWN:	(tile).offY = (tile).offY +1;	break;
    			case UP:	(tile).offY = (tile).offY -1;	break;
    			case NORTH: (tile).offZ = (tile).offZ +1;	break;
    			case SOUTH: (tile).offZ = (tile).offZ -1;	break;
    			case WEST:  (tile).offX = (tile).offX +1;	break;
    			case EAST:	(tile).offX = (tile).offX -1;	break;
    			default: 
    			}
