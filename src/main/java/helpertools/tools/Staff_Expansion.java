...
public class Staff_Expansion extends ItemTool
{
	
	
    public Staff_Expansion(ToolMaterial material, String name)
    {
    	super (1F, material, ModRegistry.properharvest);
        this.maxStackSize = 1;  
...
/////////////////////////////////////////////////////////////////////
  public void onUpdate(ItemStack stack, World world, Entity entity, int p_77663_4_, boolean p_77663_5_) {
	...
	public void Placement (ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int theface){
				...
				|| world.getBlock(x, y, z) == Blocks.snow_layer)
        {
			...
			ItemStack stacky = new ItemStack (Item.getItemFromBlock(returnTBlock(stack)),0, returnTMeta(stack));  
			//Item item = Item.getItemFromBlock(returnTBlock(thestaff));    
			Boolean whitelist_flag;
			whitelist_flag = Whitelist_Util.Block_Whitelist(returnTBlock(stack), player, returnTMeta(stack));
        			...
        			|| whitelist_flag)
    		{
						...
						|| world.getBlock(x, y, z) == Blocks.snow_layer) 
				{
        		 ...
        		 world.playSoundEffect((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F), returnTBlock(stack).stepSound.getStepResourcePath(), (returnTBlock(stack).stepSound.getVolume() + 1.0F) / 2.0F, returnTBlock(stack).stepSound.getPitch() * 0.8F);
        		 world.setBlock(x, y, z, Blocks.cobblestone);
        		//world.setBlock(x, y, z, Blocks.air);
        		world.setBlock(x, y, z, (returnTBlock(stack)), (returnTMeta(stack)), 012);
        		...
        		if (!player.capabilities.isCreativeMode){                	
        			...
        			if(whitelist_flag){
        				Whitelist_Util.Consume_Whitelist(stacky, player, returnTBlock(stack), returnTMeta(stack));
